...
function MaterialsPage() {
  ...
  // Buat params dengan useMemo agar referensi stabil — hanya berubah saat nilai benar-benar berubah
  const params = useMemo(() => {
    const p: Record<string, string> = {};
    if (semesterId) p.semester_id = semesterId;
    if (subjectFilter !== "all") p.subject_id = subjectFilter;
    return p;
  }, [semesterId, subjectFilter]);

  const { data, loading, reload } = useApiData<Material[]>(
    semesterId ? "/materials" : null,
    semesterId ? params : undefined
  );
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label>Kode Rapor</Label>
                <Input
                  value={form.kode_rapor}
                  onChange={(e) => setForm({ ...form, kode_rapor: e.target.value })}
                  placeholder="R1, R2..."
                />
              </div>
              <div className="space-y-2">
